...
declare module angular.route {
     ...
     * {Array.<Object>} - route parameters extracted from the current $location.path() by applying the current route
     */
    templateUrl ? : string | {($routeParams ? : ng.route.IRouteParamsService)
:
    string;
}
/**
 ...
 */
redirectTo ? : string | {($routeParams ? : ng.route.IRouteParamsService, $locationPath ? : string, $locationSearch ? : any)
:
string
}
;
/**
 * Reload route when only $loctaion.search() or $location.hash() changes.
 *
